...
import (
	"github.com/zhaopengme/leacrawler/util"
	"io/ioutil"
	"log"
	"net/http"
	"os"
...
type Crawler struct {
	...
	host               string // www.zhaopeng.me zhaopeng.me
	schemeAndHost      string // http://zhaopeng.me
	targetPath         string
...
func (this *Crawler) do(url string, needException bool) (children []string) {
	...
	if !needException && (err != nil || content == "") { // !needException防止处理异常时无限循环
		this.exceptionUrl.Store(url, true)
		return;
	}

	this.hadDoneUrl.Store(url, true)
...
func (this *Crawler) doExceptionUrl() {
	this.exceptionUrl.Range(func(url, value interface{}) bool {
...
func (this *Crawler) isNotNeedUrl(url string, needException bool) bool {
	hadDone, _ := this.hadDoneUrl.Load(url)
	if hadDone == nil {
		return false
	}
	if ok := hadDone.(bool); ok {
		return true
	}
	exception, _ := this.exceptionUrl.Load(url)
	if exception == nil {
		return false
	}
	ok := exception.(bool);
